...
func (a *ManifestAPI) GetManifest(ctx context.Context, _ *agentproto.GetManifestRequest) (*agentproto.Manifest, error) {
	...
	// nolint:gocritic // System context needed to read secrets for
	// the workspace owner.
...
func dbUserSecretsToProto(secrets []database.UserSecret) []*agentproto.WorkspaceSecret {
	...
	for _, s := range secrets {
		// Only include secrets that have an environment variable name or file
		// path set. Secrets with neither are not injected at runtime.
