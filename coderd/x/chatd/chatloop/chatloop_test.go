...
func TestStopAfterTool_ErrorDoesNotStop(t *testing.T) {
	t.Parallel()
	...
	model := &loopTestModel{
		...
		streamFn: func(_ context.Context, _ fantasy.Call) (fantasy.StreamResponse, error) {
			...
			default:
				return streamFromParts([]fantasy.StreamPart{
					{Type: fantasy.StreamPartTypeTextStart, ID: "text-1"},
					{Type: fantasy.StreamPartTypeTextDelta, ID: "text-1", Delta: "fallback response"},
					{Type: fantasy.StreamPartTypeTextEnd, ID: "text-1"},
					{Type: fantasy.StreamPartTypeFinish, FinishReason: fantasy.FinishReasonStop},
				}), nil
			}
	...
	proposePlanTool := fantasy.NewAgentTool(
		...
		"writes a plan",
		func(context.Context, struct{}, fantasy.ToolCall) (fantasy.ToolResponse, error) {
			return fantasy.NewTextErrorResponse("plan save failed"), nil
		},
	)
	...
	require.NoError(t, err)
	require.Equal(t, 2, streamCalls)
	require.GreaterOrEqual(t, persistStepCalls, 2)
	require.Len(t, persistedSteps, 2)
	...
	for _, block := range persistedSteps[0].Content {
		...
		_, isErr := toolResult.Result.(fantasy.ToolResultOutputContentError)
		foundErrorResult = isErr
	}
	require.True(t, foundErrorResult, "the first persisted step should capture the tool error result")
}

func TestStopAfterTool_NotInList(t *testing.T) {
	t.Parallel()
	...
	model := &loopTestModel{
		...
		streamFn: func(_ context.Context, _ fantasy.Call) (fantasy.StreamResponse, error) {
			...
			case 0:
				return streamFromParts([]fantasy.StreamPart{
					{Type: fantasy.StreamPartTypeToolInputStart, ID: "tc-other", ToolCallName: "other_tool"},
					{Type: fantasy.StreamPartTypeToolInputDelta, ID: "tc-other", Delta: `{}`},
					{Type: fantasy.StreamPartTypeToolInputEnd, ID: "tc-other"},
					{
						Type:          fantasy.StreamPartTypeToolCall,
						ID:            "tc-other",
			...
			default:
				return streamFromParts([]fantasy.StreamPart{
					{Type: fantasy.StreamPartTypeTextStart, ID: "text-1"},
					{Type: fantasy.StreamPartTypeTextDelta, ID: "text-1", Delta: "finished normally"},
					{Type: fantasy.StreamPartTypeTextEnd, ID: "text-1"},
					{Type: fantasy.StreamPartTypeFinish, FinishReason: fantasy.FinishReasonStop},
				}), nil
			}
	...
	err := Run(context.Background(), RunOptions{
		Model: model,
		Messages: []fantasy.Message{
			textMessage(fantasy.MessageRoleUser, "use another tool"),
		},
	...
	require.NoError(t, err)
	require.NotErrorIs(t, err, ErrStopAfterTool)
	require.Equal(t, 2, streamCalls)
	require.Equal(t, 2, persistStepCalls)
	require.Len(t, persistedSteps, 2)
}
